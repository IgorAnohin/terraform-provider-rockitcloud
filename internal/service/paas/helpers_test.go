...
func TestValidatePrometheusNotificationChannelResourceData(t *testing.T) {
	...
	for name, testCase := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			err := validatePrometheusNotificationChannelResourceData(resourceData)
...
func TestExpandPrometheusNotificationChannelParameters(t *testing.T) {
	...
	maxAlerts, ok := got["maxAlerts"].(int64)
	if !ok {
		t.Fatalf("expected maxAlerts to be int64, got %T", got["maxAlerts"])
	}
	if maxAlerts != 10 {
